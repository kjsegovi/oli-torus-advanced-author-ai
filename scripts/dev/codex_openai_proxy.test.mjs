...
const bridgePath = fileURLToPath(new URL('./codex_openai_proxy.mjs', import.meta.url));
...
async function fakeCodex(tmpDir, { auth = 'chatgpt', delayMs = 0 } = {}) {
  ...
  const authText = auth === 'chatgpt' ? 'Logged in using ChatGPT' : 'Logged in using an API key';
...
async function startBridge(t, options = {}) {
  ...
  await new Promise((resolve, reject) => {
    const timer = setTimeout(() => reject(new Error('bridge startup timed out')), 5_000);
...
async function startRealBridge(t) {
  ...
  await new Promise((resolve, reject) => {
    const timer = setTimeout(() => reject(new Error('real bridge startup timed out')), 5_000);
    child.stdout.on('data', (chunk) => {
      if (chunk.toString().includes('listening on')) {
        clearTimeout(timer);
        resolve();
...
test('modern and legacy tool-call responses retain IDs and arguments', () => {
  ...
  const modern = asChatCompletion(result, 'codex-proxy/gpt-5.6-sol', usage, true);
  ...
  assert.equal(modern.choices[0].message.tool_calls[0].function.name, tool.name);
  assert.deepEqual(
    JSON.parse(modern.choices[0].message.tool_calls[0].function.arguments),
    result.arguments,
  );

  const legacy = asChatCompletion(result, 'codex-proxy/gpt-5.6-sol', usage, false);
...
test('the structured schema binds each tool name to its own argument contract', () => {
  ...
  assert.deepEqual(schema.properties.arguments.anyOf[0].required, ['candidate']);
  assert.equal(schema.properties.arguments.anyOf[0].additionalProperties, false);
...
test('a fake Codex executable verifies readiness, modern history, research, and usage', async (t) => {
  ...
  assert.deepEqual(await health.json(), { auth_method: 'chatgpt', code: 'ready', ok: true });
  ...
  assert.ok(captures[1].args.some((value) => value.includes('allowed_domains')));
});
...
test('health rejects API-key authentication', async (t) => {
  ...
  assert.deepEqual(await response.json(), { code: 'api_key_authenticated', ok: false });
});
